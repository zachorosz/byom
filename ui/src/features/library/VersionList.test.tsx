...
describe('<VersionList />', () => {
  ...
  test('it renders nothing when the release has no alternate versions', () => {
    const { container } = render(() => <VersionList versions={[primary]} currentId="a1" />);
    expect(container).toBeEmptyDOMElement();
  });

  test('it renders nothing before the versions have loaded', () => {
    const { container } = render(() => <VersionList versions={undefined} currentId="a1" />);
    expect(container).toBeEmptyDOMElement();
  });
  ...
  test('it marks the version being viewed', () => {
    ...
    expect(getByText('ATO Edition').closest('li')).toHaveAttribute('aria-current', 'true');
    expect(getByText('Polygondwanaland').closest('li')).not.toHaveAttribute('aria-current');
  });
  ...
  test('it links other versions to their detail page', () => {
    ...
    expect(getByRole('link', { name: /ATO Edition/ })).toHaveAttribute('href', '/albums/a2');
  });
  ...
  test('the version being viewed is not a link', () => {
    ...
    expect(queryByRole('link', { name: /Polygondwanaland/ })).not.toBeInTheDocument();
  });
  ...
  test('it marks which version the library treats as primary', () => {
    ...
    expect(getByText('Polygondwanaland').closest('li')).toHaveTextContent('PRIMARY');
  });
});
