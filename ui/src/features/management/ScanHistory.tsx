...
function ScanRow(props: { scan: Scan }) {
  const failed = () =>
    props.scan.state === ScanState.FAILED || props.scan.state === ScanState.ABORTED;

  return (
    <div class="text-faint flex flex-wrap gap-x-4 py-1 font-mono text-[10px]">
      <span class={failed() ? 'text-danger' : ''}>{STATE_LABELS[props.scan.state]}</span>
      <span>{relativeTime(props.scan.finishTime ?? props.scan.startTime)}</span>
      <span>{Number(props.scan.progress?.dirsSeen ?? 0).toLocaleString()} dirs</span>
      <span>{Number(props.scan.progress?.filesSeen ?? 0).toLocaleString()} files</span>
...
export default function ScanHistory(props: ScanHistoryProps) {
  ...
  // yields undefined.
  const scans = createMemo(async () => (await listScansFor(props.locationId)).items);
      ...
      >
        ...
        <Show when={props.expanded}>
          <div class="border-line mt-1.5 border-t pt-1.5">
            <For each={scans().slice(1)}>{(scan) => <ScanRow scan={scan} />}</For>
          </div>
        </Show>
      </Show>
    </div>
