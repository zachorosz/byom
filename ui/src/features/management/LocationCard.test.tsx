...
const scan = (over: Partial<Scan>): Scan =>
  ({
    ...
    error: '',
    finishTime: { seconds: BigInt(Math.floor(Date.now() / 1000)) - 172_800n, nanos: 0 },
    progress: { dirsSeen: 412n, dirsMissing: 0n, filesSeen: 3118n, filesMissing: 0n },
    ...over,
  }) as Scan;

/**
 * card mounts LocationCard inside a router. ScanHistory calls a query(), and
 * query() reads router context — unprovided, Solid 2.0 throws ContextNotFoundError
 * rather than returning undefined, which <Loading> cannot catch.
 */
...
describe('<LocationCard />', () => {
  ...
  test('an idle source offers a Scan action', async () => {
    ...
    expect(await findByRole('button', { name: 'Scan' })).toBeInTheDocument();
  });
  ...
  test('it summarises the last scan with state, age and counts', async () => {
    fakeManagement({ listScans: () => ({ items: [scan({})], nextPageToken: '' }) });
    const { findByText } = card(undefined);
