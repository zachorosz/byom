...
const scan = (over: Partial<Scan> = {}): Scan =>
  ({
    ...
    error: '',
    startTime: { seconds: BigInt(Math.floor(Date.now() / 1000)) - 494n, nanos: 0 },
    progress: { dirsSeen: 1204n, dirsMissing: 0n, filesSeen: 18332n, filesMissing: 0n },
    ...over,
  }) as Scan;

describe('<ScanPanel />', () => {
  test('it shows the live counters', () => {
    const { getByText } = render(() => <ScanPanel scan={scan()} onCancel={() => {}} />);
  ...
  test('the progress bar is indeterminate — it carries no value attributes', () => {
    const { getByRole } = render(() => <ScanPanel scan={scan()} onCancel={() => {}} />);
  ...
  test('cancel calls back', () => {
    ...
    const { getByRole } = render(() => <ScanPanel scan={scan()} onCancel={onCancel} />);
  ...
  test('a cancelling scan disables the cancel button', () => {
    const { getByRole } = render(() => (
      <ScanPanel scan={scan({ state: ScanState.CANCELLING })} onCancel={() => {}} />
    ));
