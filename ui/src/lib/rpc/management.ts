...
export const listScansFor = query(
  (locationId: string) => rpc.management.listScans({ locationId, pageSize: 20 }),
  'scans',
);

/** scanLocation starts a scan and returns it. */
export function scanLocation(locationId: string) {
  return rpc.management.scanLocation({ locationId });
}
